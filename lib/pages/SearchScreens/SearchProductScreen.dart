      ...
      appBar: AppBar(
        actions: <Widget>[
          ...
          /*new IconButton(
            ...
            onPressed: () {
              navigatePush(AdvanceSearchScreen(
...
//                            discountTextForVerticalList(index)
  ...
  void onTapFavorite(int index, ProductListModel productList) {
    if (productList.attributes.length == 0 && (productList.isGiftCard != null && !productList.isGiftCard)) {
        Constants.progressDialog1(true, context);
...
//      isLoading = true;
  ...
  Future onClickSearchButton() async {
    if (searchTextController.text != null &&
        searchTextController.text.length > 3) {
      Constants.progressDialog1(true, context);
  ...
  Future callApiForAdvanceSearch() async {
    ...
    Map result = await SearchProductParser.searchCallApi(
        "${Config.strBaseURL}products/advancedsearch?q=" +
            searchTextController.text.toString(),
        strJson);
  ...
  Future callApi() async {
    ...
    if (result["errorCode"] == "0") {
      searchedList = result["value"];
//      searchedList.add(result["value"]);
      if (searchedList != null && searchedList.length != 0) {
        isAppBar = true;
        rangeSliders = rangeSliderDefinitions(searchedList[0].minimumPrice,
            searchedList[0].maximumPrice, searchedList[0].currencyType);
      }
    } else {
      String error = result["msg"];
